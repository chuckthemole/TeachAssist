...
def create_review(request, destination_id):
    if request.method == "POST":
        ...
        if not request.POST["title"] and not request.POST["feedback"]:
            return render(request, "teach/review/create_review.html", {"user":user, "destination":destination, "error":"Please fill in all required fields"})
        else:
        ...
        try:
            review = Review.objects.create(teacher=teacher, destination=destination, feedback=feedback, title=title, stars=stars)
            review.save()
            ...
            return render(request, "teach/review/create_review.html", {"error":"Can't create the destination"})
...
def show_review(request, review_id):
    if request.method == "GET":
        ...
        if not user.is_authenticated:
            ...
            return render(request, "teach/review/show_review.html", {"user":user, "review":review, "comments":comments})
